...
String SERVER_IP = "192.168.5.10"; // Flask服务器IP（需要改为您电脑的IP）
int SERVER_PORT = 8080;            // Flask服务器端口
...
void saveWiFiToEEPROM() {
  ...
  // 保存日志服务器配置
  EEPROM.write(401, SERVER_IP.length()); // 保存IP地址长度
  for (int i = 0; i < SERVER_IP.length(); i++) EEPROM.write(402 + i, SERVER_IP[i]); // 保存IP地址
  EEPROM.write(402 + SERVER_IP.length(), SERVER_PORT); // 保存端口号
...
void readWiFiFromEEPROM() {
  ...
  // 读取日志服务器配置
  int ipLen = EEPROM.read(401); // 读取IP地址长度
  SERVER_IP = "";
  for (int i = 0; i < ipLen; i++) SERVER_IP += char(EEPROM.read(402 + i)); // 读取IP地址
  SERVER_PORT = EEPROM.read(402 + ipLen); // 读取端口号
...
/**
 * 处理日志服务器配置请求（Web API）
 * 请求参数：ip=服务器IP，port=服务器端口
 * 返回结果："配置成功"或"配置失败"
 */
void handleLogServerConfig() {
  if (server.hasArg("ip") && server.hasArg("port")) {
    SERVER_IP = server.arg("ip"); // 获取服务器IP
    SERVER_PORT = server.arg("port").toInt(); // 获取服务器端口
    ...
    server.send(200, "text/plain", "配置成功"); // 返回结果
  } else {
    server.send(400, "text/plain", "参数错误"); // 参数缺失，返回错误
  }
}
...
void handleRoot() {
  ...
  html += "<div>WiFi设置</div>";
  html += "</a>";
  html += "<a href='#logconfig' class='sidebar-item' onclick='switchPage(\"logconfig\")'>";
  html += "<div class='sidebar-icon'>⚙️</div>";
  html += "<div>日志设置</div>";
  html += "</a>";
...
html += String("<div>WiFi连接状态：") + (isWiFiConnected ? "已连接" : "未连接") + "</div>";  html += "<div>访问地址：" + (isWiFiConnected ? WiFi.localIP().toString() : WiFi.softAPIP().toString()) + "</div>";
  ...
  // 日志设置页面
  html += "<div id='logconfig' class='page'>";
  html += "<h1 style='margin-bottom: 30px; color: #111827;'>日志设置</h1>";
  html += "<div class='card'>";
  html += "<div class='card-title'>";
  html += "<span>日志服务器配置</span>";
  html += "</div>";
  html += "<div style='line-height:1.8; color:#4B5563;'>";
  html += "<div style='margin-bottom: 10px;'>";
  html += "<label style='display: inline-block; width: 100px;'>服务器IP：</label>";
  html += "<input type='text' id='logServerIP' value='" + SERVER_IP + "' style='padding: 8px; width: 200px; border: 1px solid #E5E7EB; border-radius: 4px;'>";
  html += "</div>";
  html += "<div style='margin-bottom: 10px;'>";
  html += "<label style='display: inline-block; width: 100px;'>服务器端口：</label>";
  html += "<input type='text' id='logServerPort' value='" + String(SERVER_PORT) + "' style='padding: 8px; width: 100px; border: 1px solid #E5E7EB; border-radius: 4px;'>";
  html += "</div>";
  html += "<button onclick='submitLogServerConfig()' style='padding: 8px 16px; background-color: #0078D4; color: white; border: none; border-radius: 4px; cursor: pointer;'>";
  html += "保存配置";
  html += "</button>";
  html += "</div>";
  html += "</div>";
  ...
  // 日志服务器配置提交函数
  html += "async function submitLogServerConfig() {";
  html += "const ip = document.getElementById('logServerIP').value;";
  html += "const port = document.getElementById('logServerPort').value;";
  html += "if (!ip || !port) { alert('请输入完整的服务器地址'); return; }";
  html += "const response = await fetch(`/api/logserver/config?ip=${encodeURIComponent(ip)}&port=${encodeURIComponent(port)}`);";
  html += "const result = await response.text();";
  html += "alert(result);";
  html += "if (result === '配置成功') {";
  html += "// 更新查看日志链接";
  html += "const logLink = document.querySelector('.sidebar-item[href*=" + String(SERVER_IP) + "]');";
  html += "logLink.href = 'http://' + ip + ':' + port;";
  html += "}";
  html += "}";
