...
html += String("<div>WiFi连接状态：") + (isWiFiConnected ? "已连接" : "未连接") + "</div>";  html += "<div>访问地址：" + (isWiFiConnected ? WiFi.localIP().toString() : WiFi.softAPIP().toString()) + "</div>";
  ...
  html += ".then(data => {";
  html += "const powerStatusBox = document.querySelector('.power-status-box');";
  html += "const currentTimeEl = document.querySelector('.card-title span:nth-child(2)');";
  html += "if (powerStatusBox) {";
  html += "powerStatusBox.textContent = data.powerStatus;";
  html += "powerStatusBox.style.backgroundColor = data.statusColor;";
  html += "}";
  html += "if (currentTimeEl) {";
  html += "currentTimeEl.textContent = data.currentTime;";
  html += "}";
  html += "});";
  html += ".catch(error => console.error('更新状态失败:', error));";
  html += "}";
  html += "// 页面加载时更新一次状态";
...
void handleStatus() {
  ...
  json += "\"isWiFiConnected\":" + (isWiFiConnected ? "true" : "false") + ",";
  json += "\"checkInterval\":" + String(CHECK_INTERVAL) + ",";
  json += "\"ipAddress\":\"" + (isWiFiConnected ? WiFi.localIP().toString() : WiFi.softAPIP().toString()) + "\"";
  json += "}";
